...
public class OnlineBatch {
	...
	public OnlineBatch(SamplePlacementSimulator simulator, int numOfTrials) {
...
//			this.processCostTrials.add(i, 0.0);
	...
	public void run(int numOfTrials, int period) {
		...
		for (int trial = 0; trial < numOfTrials; trial++) {
			...
			for (int i = 0; i < this.K; i ++) {// initialize the popularity data.  
				...
				for (Dataset ds : allDatasets) {
					popularityTS.put(ds, period * RanNum.getRandomDoubleRange(1, 0));
				}
			}
			
			//Map<Integer, Map<Dataset, Double>> predictedPopularities = new HashMap<Integer, Map<Dataset, Double>>();
			
			double threshold = Parameters.maxNumOfQueriesPerTS * 0.5;// dataset is requested by 50% of the requests in each period. 
			...
			int timeslot = 0; 
			Map<Dataset, Double> currentPopularity = predictPopularity(allDatasets, historicalPopularities, timeslot);

			while (timeslot < Parameters.max_timeslots) {// process queries one by one
				...
				heuAlg.run(currentPopularity, threshold, timeslot, 1);
				...
				for (Query query : simulator.getQueriesOnlineBatch().get(timeslot).get(trial)) {
					
					if (null == historicalPopularities.get(timeslot + this.K))
						historicalPopularities.put(timeslot + this.K, new HashMap<Dataset, Double>());
					
					for (Dataset ds : query.getDatasets()) {
						DSsWithUpdatedPop.add(ds);
						
						if (null == historicalPopularities.get(timeslot + this.K).get(ds))
							historicalPopularities.get(timeslot + this.K).put(ds, 0d);
						
						historicalPopularities.get(timeslot + this.K).put(ds, historicalPopularities.get(timeslot + this.K).get(ds) + 1d);
					}
				}
				
				for (Dataset ds : allDatasets) {
					if(!DSsWithUpdatedPop.contains(ds))
						historicalPopularities.get(timeslot + this.K).put(ds, 0d);// no queries access these datasets.
				}
				timeslot ++; // each time slot has one query
				
				currentPopularity = predictPopularity(allDatasets, historicalPopularities, timeslot);
	...
	public Map<Dataset, Double> predictPopularity(//Map<Integer, Map<Dataset, Double>> predictedPopularities, 
		...
		for (Dataset ds : datasets) {
			...
			for (int i = 1; i <= this.K; i ++) {
				popularity += this.varsAR[i - 1] * historicalPopularities.get(this.K + currentTimeSlot - i).get(ds);
			}
